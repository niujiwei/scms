...
<script type="text/javascript">
    ...
    //
    function addWaybillAdjustment() {
        var row = $("#dg").datagrid('getSelections');
        if (row.length != 1) return $.messager.alert("运单调整", "请选择一行信息", "info");
        if (row[0].waybill_state == '1') return $.messager.alert("运单调整", "该运单已经进行了调整申请，请等待处理", "info");
        var shiping_order_id = row[0].shiping_order_id;
        dialog = parent.jy.modalDialog({
            title: '运单调整',
            url: 'waybillAdjustment.do?method=toAddWaybillAdjustmentInfo&shiping_order_id=' + shiping_order_id,
            width: 750,
            height: 600,
            buttons: [{
...
<form action="shipOrder.do" id="outputform" method="post">
        ...
        <div id="searchInfoId" style="display: none">
            ...
            <input type="text" class="search-text" style="width:150px"
                   id="end_address" name="end_address"> <label class="divlabel"
                                                               style="padding-left: 0px;">发货客户名称:</label> <input
                type="hidden"
    ...
    <div id="tb">
        <a href="javascript:void(0)" class="easyui-linkbutton"
           ...
           onclick="addWaybillAdjustment()">运单调整</a>
    </div>

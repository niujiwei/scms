...
<html>
  ...
  <script type="text/javascript">
 		...
 		var grid;
  		$(function(){
  			grid=$("#dg").datagrid({
  			url : 'orderC.do?method=getOrderCustom',
  			border : false,
  			rownumbers : true,
  			fit : true,
  		...
  		/* 	remoteSort:true, */
  			...
  			columns : [[
  				...
  				{field : 'receipt_time',title : '车到日期',width : 125,align:'center',
  					formatter: function(value,row,index){
  				if(value!=undefined){
  				value=value.substr(0,19);
  				}
  	...
  	function searchCar_owner(){
	 	 ...
	 	 /* 		name:$.trim($("#ddId").val()), */
  				...
  				//institution:$.trim($("#cc3").combotree("getValue")),
  				 ...
  				 is_arrive:$.trim($("#is_arrive").combobox('getValue'))
	 	});	  
  			...
  			$.ajax({
						...
						data:{del:checkarray.join(",")},
			  				success:function(data){
			  					if (data.flag) {
									parent.$.messager.confirm("信息删除","选择客户车辆下有订单信息，确定要删除？",
										function(r) {
										if(r){
												$.ajax({
	 ...
	 var myform=document.forms[1];
  	...
  	function searchMsg() {
  			...
  			if(row.length==1){
		  		...
		  		$.messager.alert("订单导入","请选择一行信息","info");	
		  		}		
	};
 	function add() {
  	...
  	<form action="shipOrder.do" id="outputform" method="post">
    	<div region="north" title="检索" class="st-north" border="false" collapsible="false"  height="60px">
			...
			<input type="text" class="search-text" id="driver_name" style="width:128px">
			 <lable>到 达:</lable>
			<select id='is_arrive' class="easyui-combobox" name="gender" style="width:128px;height:22px"
    	...
    	<div id="tb">
		  <!-- <a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true"onclick="printShipOrder()">一维码打印</a>

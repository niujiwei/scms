...
<head>
    ...
    <script type="text/javascript">
        ...
        var submitForm = function($dialog, $dg, $pjq) {
            ...
            var message = type=="1"?"确定要添加补助信息？":"确定要审核补助信息";
            $pjq.messager.confirm('导入补助信息',message, function(r) {
                if (r) {
                    ...
                    if (getdian > 0) {
                        ...
                        if (hz == ".xlsx" || hz == ".xlsm" || hz == ".xltx"
                            ...
                            || hz == ".xls") {
                            $("#impform").ajaxSubmit({
                                url : "subsidyInfo.do?method=startImportSubsidy",
                                type : "post",
                                ...
                                error : function(data) {
                                    parent.$.messager.alert('导入补助信息', '导入出错了', 'info');
                                }
                            });

                        } else {
                            $pjq.messager.alert('导入补助信息', '请选择正确的文件格式', 'info');
                        }
                    } else {
                        $pjq.messager.alert('导入补助信息', '没有后缀名的文件', 'info');
                    }
                }
            });
            ...
            function upload_success(filename) {
                if (filename != null || filename != "") {
                    ...
                    $.ajax({
                        type : "POST",
                        url : 'subsidyInfo.do?method=saveImportSubsidyInfo',
                        data : {
                            filename : filename,
                        ...
                        success : function(msg) {
                            var mssage = eval('(' + msg + ')');
                            var zs = mssage.totalNumber;//
                            var cg = mssage.numberOfEntries;
                            var list = mssage.messages;
                            if (list.length > 0) {
                            ...
                            parent.$.messager.alert('导入补助信息成功', ms, 'info');
            ...
            <form action="shipOrder.do?method=startimplinplan" id="impform">
                <input type="text" id="filename" class="filename"
                       readonly="readonly" /> <input type="button" name="file"
                                                     class="button" value="选择文件" />
                <input type="file" name="files" size="30" />
                <br/>
                <select id='type' name="result" class="easyui-combobox"
                        style="width:150px;height:22px"
                        data-options="panelHeight : 'auto',editable:false">
                    <option value="" >审核补助信息</option>
                    <option value="1">添加补助信息</option>
                </select>
            </form>
        </div>
    </div>

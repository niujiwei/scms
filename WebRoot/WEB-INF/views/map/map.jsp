...
var grid;
							...
							columns : [ [
									{
										...
										formatter : function(val, row, index) {
											if (row) {
												if (row.gpsstate == "APP") {
												   now =getServerDate().Format("yyyy-MM-dd hh:mm");
												   nowdate = new Date(row.nowdate).Format("yyyy-MM-dd hh:mm");
												    if(nowdate==now){
												      return "<div title='APP 在线' style='width: 16px;height: 16px;background:#56abe4; -moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;' ></div>";
												    
												    }else{
												      return "<div title='APP 离线' style='width: 16px;height: 16px;background:#eb4f38; -moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;' ></div>";
												    
												    }
												} else {
													if (val == 3 || val == "" || val==2) {
														return "<div title='GPS 离线' style='width: 16px;height: 16px;background:#cccccc; -moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;' ></div>";
													} else {
														return "<div title='GPS 在线' style='width: 16px;height: 16px;background:#11cd6e; -moz-border-radius: 8px;-webkit-border-radius: 8px; border-radius: 8px;  margin: 0 auto;' ></div>";
													}
									...
									},
									{
										...
										formatter : function(val, row, index) {
											...
											if (row.carno == ""
													|| row.carno == null) {
												return "<a href='javascript:void(0)' style='color:red' title='本地无此车辆，请注意维护' onclick='showcarofpoint(this)'>"
														+ val + "</a>";
											} else {

												return '<a href="javascript:void(0)" onclick=\'getDriverList('
														+ data
														+ ')\'>'
														+ val
														+ '</a>';
				...
				.combotree(
						{
							...
							onBeforeLoad : function(node, param) {
								if (node == null) {
									...
									$('#ccs').combotree('tree').tree('options').url = "depn.do?method=getTree&&id="
											+ node.id;//加载下层节点
								}
	...
	function searchcarno() {
		...
		if ($("#name_search").val() != null) {
			driverName = $("#name_search").val();
		}
		if ($("#phone_search").val() != null) {
			driverPhone = $("#phone_search").val();
		}
		$("#dg").datagrid('load', {
			...
			shiping_order_goid:$("#shiping_order_goid").val() */
	...
	function getDriverList(data) {
		...
		if (data.flag == "2") {
			$.getJSON("map.do?method=getSupplerDriver", {
				"userId" : data.carno,
			}, function(jsons) {
				if(jsons){
				  $('#dg').datagrid('loadData',jsons.list);  
				}
	...
	function cleanall() {
		...
		$("#phone_search").val("");
		$("#name_search").val("");
		$("#user_name").val(""), $("#user_address").val(""), $(
...
<body>
	<div class="easyui-layout" data-options="fit: true">
		<div region="west" border="false" split="false" title="检索信息"
			class="cs-west" style="width:300px;overflow-x: hidden;" id="westId">
			<!-- <a href="javascript:void(0) " onclick="showcarofpoint(this)">鲁Ae3245</a> -->
			<div id="alls">
				<table id="mytables">
				   <!--  <tr>
						<td>货运编号：</td>
						<td><input  id="shiping_order_num"
							style="width: 85px;" />
							<input  id="shiping_order_goid"
							style="width: 85px;" />
						</td>
					</tr> -->
				    <tr>
					...
					<tr>
						...
						<td><input id="phone_search" style="width: 175px"></td>
					</tr>
					<tr>
					   <td>
						终到位置：
						</td>
						<td><input id="user_address" style="width: 175px"></td>
					<!-- 	<td>
						所属机构:
						</td>

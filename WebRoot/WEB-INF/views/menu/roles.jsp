...
<script type="text/javascript">
	...
	$(function() {
		...
		for ( var i = 0; i < fs.length; i++) {
			if (fs[i] == "添加") {
				...
				$("#tbAppshouquan").removeAttr("style");
			}
		}
	...
	// APP授权
	function appshouquan() {
	    var row = $("#dg").datagrid('getSelections');
		if (row.length == 1) {
			$("#dlg3").dialog('open');
			var roleid = row[0].id;
			$("#hiddenid3").val(roleid);
			$.getJSON("role.do?method=getAPPRoleTree&&roleId=" + roleid + "", function(
					data) {
				$('#apptree').tree({
					checkbox : true,
					data : data,
	...
	function appUpdate() {
		var chackArray = [];
		var id = $("#hiddenid3").val();
		var ckNode = $('#apptree').tree('getChecked');
		for ( var s = 0; s < ckNode.length; s++) {
			chackArray.push(ckNode[s].id);
		}
		$.ajax({
			type : "POST",
			async : false,
			url : 'role.do?method=insertAPPRole',
			data : {
				id : id,
				ckNodes : chackArray.join(",")
			},
			success : function(data) {
			    $.messager.alert("角色管理", "APP授权成功", "info");
			    $("#dlg3").dialog('close');
...
<body class="easyui-layout">
	<div region="north" title="检索" class="st-north" border="false"
		...
		<label>角色名：</label><input id="role_name" type="text"
			...
			onclick="searchRole()" data-options="iconCls:'icon-search'"
			id="searchId">查询</a>
	</div>
	...
	<div id="dlg3">
		<div class="easyui-layout" data-options="fit:true">
			...
			<div data-options="region:'center'">
				<input type='hidden' id="hiddenid3" name='id'> <input
					type='hidden' id="hiddenmenuid3">
				<table id="dg2"></table>
			</div>
			<div id="buttons3">
				<input type="button" onclick="appUpdate()" value="授权" class="btncss"
					id="update" />
				<!-- <input type="button" onclick="closeps('update');" value="关闭" class="btncss" /> -->
			</div>
		</div>
	...
	<div id="tb">
		...
		<a href="#" class="easyui-linkbutton" id="tbshouquan"
			...
			style="display: none">授权</a> 
		<a href="#" class="easyui-linkbutton" id="tbAppshouquan"
			onclick="appshouquan()" data-options="iconCls:'icon-edit',plain:true"
			style="display: none">APP授权</a> 

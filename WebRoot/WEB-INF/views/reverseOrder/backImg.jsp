  ...
  <script type="text/javascript">  
  	var pid='<%=pid%>';
 		 ...
 		 var submitForm= function($dialog, $grid, $pjq, $mainMenu) {
		 	$pjq.messager.confirm('图片上传', '确定要上传吗?', function(r) {		 	
					if (r) {
						$.ajax({
							type : "POST",
							url : 'reverse.do?method=savepit',
							data :{ids:pid},
							success : function(msg) {
								if (msg.flag) {
									$pjq.messager.alert('图片上传', '图片上传成功','info');
									$dialog.dialog('close');
									$grid.datagrid('reload');
								} else {
									$pjq.messager.alert('图片上传', '图片上传失败','info');
								}
								;
							}
						});

(function($, window, document, undefined) {
	$.fn.tab = function(options) {
		...
		this.each(function() {
			...
			function Move() {
				...
				oThis.find(".pj-tab").eq(oindex).show().siblings(".pj-tab")
						.hide();
				mun = oindex
			}
			if (str.mark == "true") {
	...
	$.fn.CutString = function(options) {
		...
		this.each(function() {
			var oThis = $(this);
			oThis.each(function() {
				var oText = $(this).text().substring(1, str.numbers)
						+ str.lastStr;
				$(this).text(oText)
			})
		});
	...
	$.fn.Carousel = function(options) {
	    ...
	    this.each(function() {
			...
			oThis.each(function() {
				...
				var colorBg = oThis.find(".pj-Carousel-color")
						.css("background");
				oThis.find(".pj-Carousel-color").eq(0).addClass("active");
				oActive.css("marginLeft", (oboxWidth - oActive.width()) / 2
						+ "px");
				...
				function Toevevt() {
					var oIndex = $(this).index();
					$(this).addClass("active").siblings(".pj-Carousel-color")
							.removeClass("active");
					if (str.fade == "true") {
						oThis.find(".pj-Carousel-item").eq(oIndex).fadeIn(
								str.WidthMax).siblings(".pj-Carousel-item")
								.fadeOut(str.WidthMax)
					} else {
						oCarouseL.animate({
				...
				function publicFn() {
					oThis.find(".pj-Carousel-color").eq(num).addClass("active")
							.siblings(".pj-Carousel-color").removeClass(
									"active");
					if (str.fade == "true") {
						oThis.find(".pj-Carousel-item").eq(num).fadeIn(
								str.AnTimer).siblings(".pj-Carousel-item")
								.fadeOut(str.AnTimer)
					} else {
						oCarouseL.animate({

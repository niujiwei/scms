...
map.addEventListener("zoomend", function() {
	...
	/*
	 * if(map.getZoom()<11){ //getfromid("slideThree2").checked=false;
	 * //hidecarno(); }
	 */
...
/*
 * // 创建mark自定义图片 var nullcarIcon = new BMap.Icon("images/kc.png", new
 * BMap.Size(34,35), {// 行驶图片 imageOffset : new BMap.Size(0, 0), anchor : new
 * BMap.Size(20, 20) }); var carIcon = new BMap.Icon("images/zc.png", new
 * BMap.Size(34,35), { // 静止图片 imageOffset : new BMap.Size(0, 0), anchor : new
 * BMap.Size(20, 20) }); var nocarIcon = new BMap.Icon("images/lx.png", new
 * BMap.Size(34,35), {// 离线图片 imageOffset : new BMap.Size(0, 0), anchor : new
 * BMap.Size(20, 20) });
 */
var nullcarIcon = new BMap.Icon("images/gps_zx.png", new BMap.Size(34, 35), {// 行驶图片
...
var nocarIcon = new BMap.Icon("images/gps_lx.png", new BMap.Size(34, 35), {// 离线图片
	imageOffset : new BMap.Size(0, 0),
	anchor : new BMap.Size(20, 20)
});
var appLx = new BMap.Icon("images/app_lx.png", new BMap.Size(34, 35), {// app离线图片
	imageOffset : new BMap.Size(0, 0),
	anchor : new BMap.Size(20, 20)
});
var appZx = new BMap.Icon("images/app_zx.png", new BMap.Size(34, 35), {// app
																		// 在线图片
	imageOffset : new BMap.Size(0, 0),
	anchor : new BMap.Size(20, 20)
});
...
function checkusersetting() {// 检查用户按钮的控制，展示不同的数据展示
	if (_ISGT) {// 聚合判断
		...
		markerClusterer.setMinClusterSize(999999);
	}
	if (!_XSCO) {
		...
		$("#qcsl").text(
				parseInt($("#qcsl").text()) - parseInt(nullmarkers.length));
	}
	if (!_JZCO) {
	...
	if (!_LXCO) {
		...
		$("#qcsl").text(
				parseInt($("#qcsl").text()) - parseInt(nomarkers.length));
	}
	if (_ALCO) {
		for ( var z = 0; z < overlays.length; z++) {
			overlays[z].hide();
...
function createmark2(pto, num) {
    ...
    if (pto != null) {
		if (num < carAllMk.length) {
			...
			if (pto.gpsstate == "APP") {
                 now =new Date().Format("yyyy-MM-dd hh:mm");
                nowdate  = new Date(pto.nowdate)
						.Format("yyyy-MM-dd hh:mm");
				if (nowdate == now) {
					
					carAllMk[num].setIcon(appZx);
				} else {
					carAllMk[num].setIcon(appLx);
				}
...
function createmark(pto) {
	...
	if (pto != null) {
		...
		if (pto.gpsstate == "APP") {
			...
			var nowdate = new Date(pto.nowdate)
...
function careteinfor(point, carno) {
	...
	var carsno = carno.carid == undefined ? carno : carno.carid;
	var receivetime = carno.receivetime;
	var signtime = carno.signtime;
	var addComp = "";
	var add = "";
	var cityname = "";
	var weather = "";
	myGeo
			.getLocation(
					...
					function(rs) {
						...
						add = addComp.province + addComp.city
						...
						$
								.ajax({
									async : false,
									url : "http://php.weather.sina.com.cn/js.php?"
											+ $.param({
												city : cityname, // 城市
												day : 0,
									...
									success : function(json) {
										...
										weather = status1 + " " + temperature2
												+ '°~' + temperature1 + '°';
										// weather="";
										$
												.getJSON(
														"map.do?method=loadmapinfo",
														{
															"carno" : carsno
														},
														function(jsons) {
															var state = jsons[0].gpsstate=="APP"?"APP":"GPS";
															infoWindow = new BMap.InfoWindow(
																	"<table  style='  border-collapse: collapse; border-spacing: 0;line-height: 18px;white-space: nowrap;color:#333;font-size: 12px;ont-family: ''Microsoft YaHei',微软雅黑,Arial,Helvetica,Sans-Serif''> <tr> <td>用户名：</td><td>"
																			+ carsno
																			+ "</td> </tr><tr> <td>所属机构：</td><td>终端配送项目组业务合作</td> </tr><tr> <td>定位来源：</td><td>"+state+"</td> </tr><tr> <td>当前位置：</td><td>"
																			+ add
																			+ "</td> </tr><tr> <td>定位时间：</td><td>"
																			+ jsons[0].nowdate
																			+ "</td> </tr><tr> <td>天气状况：</td><td>"
																			+ weather
																			+ "</td> </tr><tr> <td>经纬度：</td><td>"
																			+ point.lng
																			+ ","
																			+ point.lat
																			+ "</td> </tr><tr><td colspan='2'><a id='carhf' onclick='carhf("
																			+ '"'
																			+ carsno
																			+ '","'
																			+ receivetime
																			...
																			+ signtime
																			+ '"'
																			+ ")' href='javascript:void(0);'>回放</a>&nbsp;<a onclick='cargz("
																			+ '"'
																			+ carsno
																			+ '"'
																			+ ")' href='javascript:void(0);'>跟踪</a></td></tr></table>",
																	opts);
															map.openInfoWindow(
																	infoWindow,
																	point); // 开启信息窗口
						...
						/*
						 * $.ajax({async : true,url :
						 * "map.do?method=getWeather&city="+cityname,
						 * dataType:"json", success : function(json) {
						 * //console.log(json); weather = json.txt + " " +
						 * json.tmp+ '°'; //weather = status1 + " " +
						 * temperature2+ '°~' + temperature1 + '°';
						 * $.getJSON("map.do?method=loadmapinfo",{"carno" :
						 * carsno},function(mapinfos) { infoWindow = new
						 * BMap.InfoWindow("<table style=' border-collapse:
						 * collapse; border-spacing: 0;line-height:
						 * 18px;white-space: nowrap;color:#333;font-size:
						 * 12px;ont-family: ''Microsoft
						 * YaHei',微软雅黑,Arial,Helvetica,Sans-Serif''> <tr> <td>车牌号：</td><td>" +
						 * carsno+ "</td> </tr><tr> <td>所属机构：</td><td>"+
						 * mapinfos[0].deptname + "</td> </tr><tr> <td>北斗设备号：</td><td>"+
						 * mapinfos[0].beidouid + "</td> </tr><tr> <td>北斗状态：</td><td>"+
						 * mapinfos[0].gpsstate + "km/h</td> </tr><tr> <td>当前位置：</td><td>"+
						 * add+ "</td> </tr><tr> <td>定位时间：</td><td>" +
						 * mapinfos[0].nowdate+ "</td> </tr><tr> <td>天气状况：</td><td>" +
						 * weather+ "</td> </tr><tr> <td>经纬度：</td><td>"+
						 * point.lng+ ","+ point.lat + "</td> </tr><tr><td colspan='2'><a
						 * id='carhf' onclick='carhf("+ '"'+ carsno+ '"'+ ","+
						 * '"'+ mapinfos[0].beidouid+ '"'+ ")'
						 * href='javascript:void(0);'>回放</a>&nbsp;<a
						 * onclick='cargz("+ '"'+ carsno+ '"'+ ")'
						 * href='javascript:void(0);'>跟踪</a></td></tr></table>",opts);
...
function showcarofpoint(carno) {
	...
	for ( var z = 0; z < overlays.length; z++) {
		...
		if (overlays[z].getLabel().content == carn) {
			...
			overlays[z].setZIndex(99);
			careteinfor(seach_points, carno);
		}
	}
}
...
function createcirandseachkeyname(seach_points) {
	if (sfz) {
		...
		local.setSearchCompleteCallback(function(results) {
			...
			if (local.getStatus() == BMAP_STATUS_SUCCESS) {
				for ( var i = 0; i < results.getCurrentNumPois(); i++) {
					...
					seachlabel = new BMap.Label(results.getPoi(i).title, {
						position : seach_points_label,
		...
		circle = new BMap.Circle(seach_points, 10000, {
			fillColor : "",
			strokeWeight : 4,
			fillOpacity : 0,
			strokeOpacity : 0

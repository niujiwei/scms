...
//创建mark自定义图片
var nullcarIcon = new BMap.Icon("images/kc.png", new BMap.Size(35,35), {// 行驶图片
	imageOffset : new BMap.Size(0, 0)
// 图片的偏移量。为了是图片底部中心对准坐标点。
});
var carIcon = new BMap.Icon("images/zc.png", new BMap.Size(35,35), { // 静止图片
	imageOffset : new BMap.Size(0, 0)
});
var nocarIcon = new BMap.Icon("images/lx.png",new BMap.Size(35,35), {// 离线图片
...
function getcarinfo(carno){
	...
	setTimeout("getcarinfo(_carno)",7000);
}
	...
	myGeo.getLocation(point,function(rs) {
		...
		$.ajax({async : false,url : "http://php.weather.sina.com.cn/js.php?"+ $.param({
			...
			$.getJSON("map.do?method=loadmapinfo",{"carno" : carsno},function(mapinfos) {
				infoWindow = new BMap.InfoWindow("<table  style='  border-collapse: collapse; border-spacing: 0;line-height: 18px;white-space: nowrap;color:#333;font-size: 12px;ont-family: ''Microsoft YaHei',微软雅黑,Arial,Helvetica,Sans-Serif''> <tr> <td>用户名：</td><td>"
				+ carsno+ "</td> </tr><tr> <td>所属机构：</td><td>终端配送项目组业务合作</td> </tr><tr> <td>定位来源：</td><td>手机端</td> </tr><tr> <td>当前位置：</td><td>"+ add+ "</td> </tr><tr> <td>定位时间：</td><td>"
		...
		/*$.ajax({async : true,url : "map.do?method=getWeather&city="+cityname, dataType:"json",
			success : function(json) {
				//console.log(json);
				weather = json.txt + " " + json.tmp+ '°';
				//weather = status1 + " " + temperature2+ '°~' + temperature1 + '°';
				$.getJSON("map.do?method=loadmapinfo",{"carno" : carsno},function(mapinfos) {
					infoWindow = new BMap.InfoWindow("<table  style='  border-collapse: collapse; border-spacing: 0;line-height: 18px;white-space: nowrap;color:#333;font-size: 12px;ont-family: ''Microsoft YaHei',微软雅黑,Arial,Helvetica,Sans-Serif''> <tr> <td>车牌号：</td><td>"
					+ carsno+ "</td> </tr><tr> <td>所属机构：</td><td>"+ mapinfos[0].deptname

...
public class DepartmentController {
	...
	public @ResponseBody
	Map<String, Department> getDepartmentfname(String department_id) {
		Department depn = depnService.getDepartment(department_id);
	...
	public String deleteDepartment(String father_id) {
		depnService.delDepartment(father_id);
		return "menu/departments";
	}
	...
	public @ResponseBody
	boolean checkDepn(String name, String fid) {
	...
	public @ResponseBody
	String getDepartmentTree(String id,HttpSession session) {
		User user = (User) session.getAttribute(SessionInfo.SessionName);//获取登录的用户信息
		String[] s = id.split(",");
		String tree = depnService.getDepartmentTree(s[0], user);
		return tree;
	}

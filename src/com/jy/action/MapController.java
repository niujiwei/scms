...
public class MapController {
	...
	String loadmapinfo(String carno, HttpSession session) {
		...
		List<Maps> loadmaplist = mapService.loadmapinfo(carno, user);
	...
	public @ResponseBody
	Map loadallcarinfo(String rows, String page, String driverName,
			String dept, String driverPhone, String shiping_order_num,
			String shiping_order_goid,String user_name,String user_address, HttpSession session) {
		...
		List<Maps> list = mapService.loadallcarinfo((page1 - 1) * rows1, rows1,
				driverName, dept, driverPhone, shiping_order_num,
				shiping_order_goid, user,user_name,user_address);
		int count = mapService.loadallcarinfocount(driverName, dept,
				driverPhone, shiping_order_num, shiping_order_goid, user,user_name,user_address);
	...
	public @ResponseBody
	String gethfpage(String carno, String times, HttpServletRequest request,
			HttpSession session) {
		String starttime = "";
		String endtime = "";
		if (times != null && !times.equals("")) {
			...
			System.out.println(times + "\t" + starttime + "\t" + endtime);
			int num = 0;
	...
	public @ResponseBody
	String gethfinfonozero(int page, String carno, String times,
		...
		if (times != null && !times.equals("")) {
			...
			mplist = mapService.gethfmessnozero(db[0], oldcarno, starttime,
					endtime, db[1], (page * 1000) + "");
			System.out.println("回放数据加载完了" + (new Date().getTime() - looo)
					+ "\t" + mplist.size());
			double licheng = 0;
			/*
			 * if(mplist.size()>1){
			 * if(mplist.get(0).getNowspace()!=null&&mplist.
			 * get(mplist.size()-1).getNowspace()!=null){ BigDecimal
			 * decimal2=new BigDecimal(mplist.get(0).getNowspace()); BigDecimal
			 * decimal1=new
			 * BigDecimal(mplist.get(mplist.size()-1).getNowspace());
			 * licheng=decimal1.subtract(decimal2).setScale(2,
			 * BigDecimal.ROUND_HALF_UP).doubleValue(); } }else
			 * if(mplist.size()==1){ licheng=0; }
			 */
			// BaiDuMap.licheng(mplist);
			/*
			 * Maps maps=mapService.getqtmessgage(oldcarno,dbname);
			 * if(maps!=null){ for (Maps map : mplist) {
			 * map.setDepartmentname(maps.getDepartmentname());
			 * map.setTels(maps.getTels()); //map.setNowspace(nowspace); } }
			 */
	 ...
	 * @param mess
	 */
	public void expExcel(String expFilePath, String mess,
	...
	public String getmapzz(String gzhidvalue, HttpServletRequest request) {
		try {
			if (gzhidvalue != null) {
				gzhidvalue = new String(gzhidvalue.getBytes("iso-8859-1"),
						"utf-8");
			}
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
	...
	public String getmaphf(String hzhidvalue, String receivetime,
			String signtime, HttpServletRequest request) {
		try {
			if (hzhidvalue != null) {
				hzhidvalue = new String(hzhidvalue.getBytes("iso-8859-1"),
						"utf-8");
			}
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		if (receivetime == null || receivetime.equals("0")
				|| receivetime.equals("null"))
			receivetime = "";
		if (signtime == null || signtime.equals("0") || signtime.equals("null"))
			signtime = "";
	...
	String gethfinfo(String carno, String times) {
		...
		System.out.println(carno + times + "========");
		String starttime = "";
		String endtime = "";
		if (times != null && !times.equals("")) {
			String[] str = times.split("至");
			...
			// 声明一个数组保存时间点
			/*
			 * List<String> siteList = new ArrayList<String>(); String t = "";//
			 * 记录停留点开始时间 double st = 0;// 记录停留点持续时间 for (int i = 0; i <
			 * mplist.size(); i++) { Mapsite mst=new Mapsite(); if
			 * (mplist.get(i).getGpsstate().equals("0.0")) {
			 * 
			 * // 如果连续为0时 if (i >= 1) { st = DateUtils
			 * .diffDatemin(DateUtils.parseDates(mplist.get(i)
			 * .getNowdate(),"yyyy-MM-dd HH:mm:ss"),
			 * DateUtils.parseDates(t,"yyyy-MM-dd HH:mm:ss")); } t =
			 * mplist.get(i).getNowdate(); }else{//速度不为0
			 * if(!t.equals("")){//只有一个空缺 st = DateUtils
			 * .diffDatemin(DateUtils.parseDates(mplist.get(i)
			 * .getNowdate(),"yyyy-MM-dd HH:mm:ss"),
			 * DateUtils.parseDates(t,"yyyy-MM-dd HH:mm:ss"));
			 * System.out.println(st); t=""; st=0; }else
			 * if(!t.equals("")&&st!=0){//2个或以上的空缺 st=st+DateUtils
			 * .diffDate(DateUtils.parseDate(mplist.get(i) .getNowdate()),
			 * DateUtils.parseDate(t)); System.out.println(st); t=""; st=0; } }
			 * //循环内
			 * 
			 * }
			 */
	...
	Map gethfinfopage(String carno, String times, String rows, String page) {
		...
		if (times != null && !times.equals("")) {
			...
			starttime = str[0].trim();
			endtime = str[1].trim();
			List<Maps> mplist = mapService.gethfmesspage(carno, starttime,
					endtime, (page1 - 1) * rows1, rows1);// 获取坐标点集合
	...
	@RequestMapping(params = "method=getSupplerDriver")
	public @ResponseBody
	JSONObject getSupplerDriver(String userId) {
		
		JSONObject jb = new JSONObject();
		List<Maps> maps = mapService.getSupplerDriver(userId);
		jb.put("list", maps);
		return jb;
	}
